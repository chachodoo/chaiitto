...
// ----------------------------------------------------
    ...
    container.innerHTML = html;
        ...
        if (pageName === 'productos') {
            ...
            // AMEX CENTURION: Quiet luxury on landing (confetti reserved for when they actually join)
        }
        ...
        list.forEach(prod => {
            ...
            <div style="width: 100%; margin-top: auto; padding-top: 4px;">
                ...
                <div style="display: flex; gap: 6px; justify-content: center; margin-bottom: 8px;">
                    <button type="button" onclick="event.stopPropagation(); setCardPresentation('${prodId}', 'Frasco', ${frascoPrice}, '${safeName}')" id="${prodId}_btn_frasco" style="flex: 1 1 0; min-width: 0; box-sizing: border-box; padding: 4px 2px; font-size: 0.68rem; font-weight: 800; letter-spacing: 0.3px; border-radius: 12px; border: 1.5px solid var(--matcha-deep, #07511A); background: var(--matcha-deep, #07511A); color: #FFFFFF; cursor: pointer; text-align: center; white-space: nowrap; transition: all 0.15s ease;">FRASCO</button>
<button type="button" onclick="event.stopPropagation(); setCardPresentation('${prodId}', 'Sobre', ${sobrePrice}, '${safeName}')" id="${prodId}_btn_sobre" style="flex: 1 1 0; min-width: 0; box-sizing: border-box; padding: 4px 2px; font-size: 0.68rem; font-weight: 800; letter-spacing: 0.3px; border-radius: 12px; border: 1.5px solid rgba(7, 81, 26, 0.3); background: #FFFFFF; color: var(--matcha-deep, #07511A); cursor: pointer; text-align: center; white-space: nowrap; transition: all 0.15s ease;">SOBRE</button>
                </div>` : ''}
...
/* ===================================================
   COMMUNITY FINALE: 3D CARD FLIP & RANDOM RESHUFFLE
   - Filters galeria.json: Only cc- and vv- (ignores no-)
   - Supports .mp4 autoplay & JPG/jpg/jpeg images
   - Staggered 3D flips between Chai-itto Logo & Media
   - Infinitely loops with fresh random order each cycle
   =================================================== */
let activeCommunityPool = [];
let isMosaicRunning = false;
let mosaicLoopTimer = null;
const TOTAL_MOSAIC_SLOTS = 24;

// Helper to ensure correct relative path
function formatMediaPath(item) {
  let p = typeof item === 'string' ? item : (item.src || item.image || item.file || item.url || '');
  p = p.trim();
  if (!p) return '';
...
// 1. Fetch JSON and filter only vv- and cc-, strictly ignoring no-
async function loadFilteredMedia() {
  if (activeCommunityPool.length >= TOTAL_MOSAIC_SLOTS) return;

  try {
    const res = await fetch('galeria.json?v=' + Date.now());
    if (res.ok) {
      const data = await res.json();
      const rawList = Array.isArray(data) ? data : (data.images || data.galeria || data.items || Object.values(data));
      
      activeCommunityPool = rawList
        .map(formatMediaPath)
        .filter(path => {
          const fileName = path.split('/').pop().toLowerCase();
          const isAllowedPrefix = fileName.startsWith('cc-') || fileName.startsWith('vv-');
          const isIgnored = fileName.startsWith('no-');
          return isAllowedPrefix && !isIgnored;
        });
    }
  } catch (err) {
    console.warn("Error cargando galeria.json, usando lista activa:", err);
  }

  // Safety fallback if JSON fails
    ...
    for (let i = 0; i < TOTAL_MOSAIC_SLOTS; i++) {
      ...
      tile.innerHTML = `
        <div class="tile-inner">
          <div class="tile-face tile-front" id="tile-front-${i}"></div>
...
function mountShuffledMedia() {
  const deck = [...activeCommunityPool].sort(() => Math.random() - 0.5).slice(0, TOTAL_MOSAIC_SLOTS);

  for (let i = 0; i < TOTAL_MOSAIC_SLOTS; i++) {
    const frontFace = document.getElementById(`tile-front-${i}`);
    if (!frontFace) continue;

    const mediaSrc = deck[i] || activeCommunityPool[i % activeCommunityPool.length];
...
/* ===================================================
   VIP PROFILE DRAWER SYSTEM (COMPACT & RIGHT-DOCKED)
   =================================================== */
...
function renderProfileDrawerContent() {
  ...
  if (!isAuth) {
    ...
    // 2. ACTIVE VIP MEMBER DASHBOARD (COMPACT FIT)
    const name = sessionStorage.getItem('chai_vip_name') || 'Miembro VIP Premium';
    ...
    container.innerHTML = `
      <div style="flex: 1; display: flex; flex-direction: column; justify-content: space-between; gap: 12px;">
        ...
        <!-- MEMBER INFO -->
        <div style="background: rgba(0,0,0,0.3); border: 1px solid rgba(212, 175, 55, 0.3); border-radius: 10px; padding: 12px 14px; display: flex; justify-content: space-between; align-items: center;">
          <div>
            <span style="font-size: 0.65rem; font-weight: 800; color: #F5D061; text-transform: uppercase; letter-spacing: 0.8px;">VIP Premium Activo</span>
            <div style="color: #FFFFFF; font-family: var(--font-heading, 'Cinzel', serif); font-size: 1.05rem; font-weight: 700; margin-top: 1px;">${name}</div>
            <div style="color: #A3C4AF; font-size: 0.7rem;"><i class="fa-regular fa-calendar-check" style="color: #D4AF37;"></i> ${expiry}</div>
          </div>
          <button onclick="logoutFromProfileDrawer()" style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.2); color: #FFFFFF; padding: 5px 10px; border-radius: 6px; font-size: 0.72rem; cursor: pointer;">
            Salir
          </button>
        </div>

        <!-- CUPS TRACKER -->
        <div style="background: rgba(0,0,0,0.3); border: 1px solid rgba(212, 175, 55, 0.3); border-radius: 10px; padding: 14px 12px; text-align: center;">
          <div style="font-size: 0.78rem; font-weight: 700; color: #E2ECE5; margin-bottom: 8px;">Tazas de cortesía del mes (4 al mes)</div>
          <div style="display: flex; justify-content: center; align-items: center; margin: 8px 0;">${cupsHtml}</div>
          <div style="font-size: 0.78rem; color: #F5D061; font-weight: 700; margin-top: 6px;">
            ${remaining > 0 ? `Te quedan ${remaining} tazas disponibles` : '¡Completaste tus 4 tazas del mes!'}
          </div>

          ${remaining > 0 ? `
            <button onclick="toggleProfileDrawer(); redeemVipCup();" style="margin-top: 10px; width: 100%; padding: 10px; border-radius: 8px; background: linear-gradient(135deg, #F5D061 0%, #D4AF37 100%); color: #102619; font-weight: 800; font-size: 0.88rem; border: none; cursor: pointer; box-shadow: 0 4px 12px rgba(212, 175, 55, 0.3);">
              <i class="fa-solid fa-mug-hot"></i> Canjear 1 Taza
            </button>
          ` : ''}
        </div>
